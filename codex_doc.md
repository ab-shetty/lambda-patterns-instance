...
The follow-up 100-image generation scheme is checked in under
`image_generation/`. `image_generation/README.md` is authoritative and
`prompts.jsonl` contains the exact 100 prompts. Images must be generated by an
image model, reviewed locally, and uploaded unlabelled as Roboflow batch
`realistic-label-pool-v1-100`; none of the HF evaluation images may be used.
