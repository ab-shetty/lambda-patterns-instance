...
for A in v6d hsonly subtleonly; do train $A & done; wait
...
{ echo; echo "**Split screen (auto-recorded, \`run_split_screen.sh\`, seed $SEED):**"; echo '```'; cat logs/run_split_screen_s$SEED.log; echo '```'; } >> synth_progress.md
git add synth_progress.md run_split_screen.sh && git commit -q -m "Split synth screen, all three arms (auto-recorded)
