...
set -euo pipefail
SEED=${1:-7}
declare -A POOL=([v6d]=data/synthetic/v6d_1600 [hsonly]=data/synthetic/v6dhsonly_1600 [subtleonly]=data/synthetic/v6dsubtleonly_1600)
...
for A in hsonly subtleonly; do train $A & done; wait
for A in v6d hsonly subtleonly; do python3 -c "
...
for A in hsonly subtleonly; do python3 scripts/paired_compare.py --a $E/so_v6d_s${SEED}_hf14.json \
  --b $E/so_${A}_s${SEED}_hf14.json --label-a v6d --label-b $A | sed -n '/paired mean/,/sign-test/p'; done
# record and push (the VM may be gone before anyone reads the log)
{ echo; echo "**Split screen (auto-recorded, \`run_split_screen.sh\`, seed $SEED):**"; echo '```'; cat logs/run_split_screen.log; echo '```'; } >> synth_progress.md
git add synth_progress.md run_split_screen.sh && git commit -q -m "Split synth screen: hardscape-only vs subtle-band-only (auto-recorded)
